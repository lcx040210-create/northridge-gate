...
export async function buildRoom(scene: THREE.Scene): Promise<void> {
  const [wallTex, floorTex, sofaTex, metalTex, woodTex] = await Promise.all([
  ...
  scene.add(cabinet)
  const cabinetDoor = new THREE.Mesh(new THREE.BoxGeometry(0.04, 1.6, 0.5), new THREE.MeshStandardMaterial({ color: 0x33383e, roughness: 0.6, metalness: 0.3 }))
  cabinetDoor.position.set(4.74, 1.1, 1.2)
  ...
  cabinetHandle.position.set(4.78, 1.1, 1.0)
  scene.add(cabinetHandle)

  // 手册柜（左墙）
  const shelf = new THREE.Mesh(new THREE.BoxGeometry(1.6, 2.0, 0.5), woodMat)
  shelf.position.set(-4.2, 1.0, 1.2)
  scene.add(shelf)
  const shelfBoard = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.06, 0.46), new THREE.MeshStandardMaterial({ color: 0x3f3524, roughness: 0.8 }))
  shelfBoard.position.set(-4.2, 1.45, 1.2)
  scene.add(shelfBoard)
  const shelfBoard2 = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.06, 0.46), new THREE.MeshStandardMaterial({ color: 0x3f3524, roughness: 0.8 }))
  shelfBoard2.position.set(-4.2, 0.75, 1.2)
  ...
  doorHandle.position.set(0.35, 1.1, 2.95)
  ...
  doorLock.position.set(0.05, 1.25, 2.96)
  scene.add(doorLock)
